...
## - [x] Task 2: event/logging 3.x 契约兼容改造

- [x] **2.1 — 修正 dispatcher/event 关键实现**
...
- [x] **3.2 — 全量扫描 src 与 ut 的 8.2 语法一致性**
...
## - [x] Task 6: Code Review 收口

- [x] **6.1 — 派发 code-reviewer sub-agent**
  - 修改：无（审查阶段）
  - 内容：按当前分支 diff 做逐文件审查，聚焦行为回归、边界条件、错误处理与命名一致性。
- [x] **6.2 — 修复 review 问题并复验**
  - 修改：review 指出的目标文件
  - 命令：`vendor/bin/phpunit`
  - 预期：问题关闭后测试仍全绿。
- [x] **6.3 — 最终发布前检查与提交**
...
- 2026-04-26：收官 Task 1–6：SSOT/CHANGELOG/手工测试记录、alpha `v3.0.0-alpha.1`、plan 全勾选；代码走读无新增问题（以当次合入与测试为准）。
