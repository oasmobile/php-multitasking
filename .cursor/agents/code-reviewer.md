...
- **代码风格一致性** — 命名规范、格式统一
- **命名规范** — 类名、函数名、变量名是否清晰、符合语言惯例
- **错误处理完整性** — 异常路径是否覆盖，是否有遗漏的 error handling
- **潜在的性能问题** — 不必要的循环、重复计算、内存泄漏风险
- **过大的文件或函数** — 单个函数是否过长，单个文件是否过大
- **未清理的 TODO / FIXME / 调试代码** — 不应残留在提交中
- **Code smell** — 过长函数、重复代码、过深嵌套（> 3 层）、God class、不必要的复杂度
- **Compiler warning / deprecation warning** — 确保零 warning
- **不必要的 import、未使用的变量 / 参数**
- **可见性过宽** — 应为 private / internal 的成员暴露为 public
- **安全漏洞** — 涉及外部输入、配置、认证、文件路径拼接等场景时，检查是否存在注入、路径遍历、敏感信息泄漏等风险
- **测试质量** — 新增 / 变更的测试是否覆盖关键路径、边界条件和错误场景；是否存在无断言或断言过弱的测试
- **与 design.md 的一致性** — 如果当前 spec 目录下有 design.md，检查实现是否与设计一致
